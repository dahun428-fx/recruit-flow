...
> **현행화(2026-08-23, PM 감사 반영)** — 본문은 2026-08-14 시점 기록이며
> 아래 전제가 그 후 바뀌었다:
> - **스택**: SQLite → **Postgres**(재플랫폼 Phase 1·2a, replatform-plan.md).
>   본문의 sqlite 파일·`RECRUIT_FLOW_DB_PATH` 서술은 역사 기록.
> - **회귀 게이트**: 스모크 82+38은 pg 전환으로 실행 불능 → **공식 폐기**.
>   현행 게이트 = 결정론 e2e 34 + `npm run test:unit` 27(gate 파서·JSON
>   재시도 등 순수 로직, e2e.md 상단 참조).
> - **부채 B(SSE 재개)**: `e2e/sse-recovery.spec.ts`(CDP 오프라인 강제→
>   복원·재구독)로 **해소 완료**.
> - **부채 A(실 LLM 최종 관문)**: 하단 "실증 결과(2026-08-23)" 참조.
...
## 실증 결과 (2026-08-23~24) — 부채 A: 실 LLM 최종 관문

감사 R3 조치로 격리 pg DB(`recruit_flow_r3` = 실 DB 정본 복제)에서
`scripts/r3-live-gate.mts`(러너 직접 구동, dev 서버 불요)로 실 LLM 완주를
3회 시도했다. 실 JD(빗썸페이 Frontend Engineer — React/RN·TS)를 JD 교체
루프 규약(nodes.md §3, "현재 JD" v2 등록)으로 주입, 정본 증거(프로필·
경험 뱅크)와 매칭. 증거: `tmp/r3-live/`(채점 JSON·초안·로그).

### 인프라 실증 — ✅ 완료 (감사 R2의 실경로 공백 해소)

- 실 LLM 전 경로 완주: 입력 3종 → tailor → writer(도구 장착, 회차당
  5~13분 집필) → 채점 2종 병렬 → 관문 → 재작성 루프 → maxLoops 소진 시
  gate_failed + 완성본 skip. 상태 머신·라우팅 전부 정확.
- **채점 JSON 파싱 12/12 성공(재시도 발동 0회)** — runAgentJson의
  스키마 힌트가 실 LLM에서 유효(재시도 로직 자체는 `test:unit`이 검증).
- 관문 판정 8회 전부 정확(1 pass·7 fail), gate_decision DB 영속 정상.
- 플레이스홀더 JD 대조군(시드 원본 그대로 1회차): 에이전트가 날조 없이
  "JD 없음 — 채점 불가"로 정직 FAIL → AI 사용 가드 규칙 준수 실증.

### 최종 가치(양 채점 동시 PASS → HTML) — ❌ 미달 (open 유지)

| 시도 | 회차별 (recruiter / tech) | 종단 |
| --- | --- | --- |
| 2차 | ① 78/**80 PASS** ② 59/51 (③ 도중 55분 캡) | 미종단 |
| 3차 | ① 59/59 ② **81 PASS**/76 ③ 59/59 | gate_failed |

각 축 개별 PASS는 확인(rec 81·tech 80) — **동시 PASS가 미달**. 원인은
앱이 아니라 콘텐츠·데이터(채점자 blockers가 명시):

1. **`[확인 필요]` 플레이스홀더 본문 잔존 → 59점 하드캡.** 경험 뱅크의
   미확정 사실(내담씨앤씨 RN 포함 여부, 비즈36.5 Expo/Keychain/세션 상세)
   을 writer가 `[확인 필요]`로 표기한 채 제출 — 채점자가 "제출 불가
   상태"로 정확히 캡. → **소유자 액션**: 경험 뱅크의 해당 항목 사실 확정
   (또는 해당 에이전트의 갭 인터뷰 모드를 "멈추고 묻기"로 전환).
2. **Screen profile 태그 미지정 → Balanced 프리셋 폴백.** 채점자 스스로
   "AI-Product 프리셋이면 ~82점" 추정. → **소유자 액션**:
   target-companies.md에 빗썸페이 엔트리 + 프리셋 태그 등록.
3. 재작성 루프는 비단조(피드백 반영이 점수 향상을 보장하지 않음, 실행 간
   편차 59~81) — 관문 통과는 반복 실행이 아니라 위 데이터 정비가 경로.

**판정**: M2 최종 관문은 계속 open이되, 남은 것은 검증 인프라가 아니라
소유자 데이터 정비 2건이다. 정비 후 `npx tsx scripts/r3-live-gate.mts`
재실행으로 닫는다.
