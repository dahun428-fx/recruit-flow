...
| 9b | 실행 모델 | 채팅 = 사령탑(JD·문서 등록, 블록 add, "작성해줘" = 실행 트리거). **run 시작 시 그래프 스냅샷** — 실행 중 편집은 다음 run에 반영. 인터랙티브 vs 파이어-앤-포겟은 그래프에 내 검토 노드를 넣었는지가 결정. 갭 인터뷰는 에이전트 노드 속성: "갭 발견 시 멈추고 묻기 / `[확인 필요]` 표시하고 계속" |
