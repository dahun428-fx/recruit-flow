...
## JD 교체 반복 루프 (핵심 사용 패턴)
이 도구의 주 용도는 **고정된 증거베이스(경력·프로필)에 JD만 바꿔가며 맞춤 이력서를 반복 생성**하는 것이다.
사용자가 새 JD(채용공고) 텍스트를 주면 표준 흐름은:
1. register_document(name="현재 JD", content=받은 JD 전문)로 등록한다. **이름은 항상 정확히 "현재 JD"** —
